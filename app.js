...
function renderSeasonSel(){
  ...
  $("season-sel").innerHTML='<div style="display:flex;align-items:center;gap:6px"><select class="season-sel" id="ss">'+opts+'</select>'+
    (canEdit()?'<button class="hdr-newseason-btn" id="hdr-newseason" title="Nueva temporada">+</button>':"")+"</div>";
  $("ss").addEventListener("change",function(e){S.season=e.target.value;route(S.view);});
  var nb=$("hdr-newseason");if(nb)nb.addEventListener("click",function(){openNewSeasonWizard(S.season,nextSea(S.season));});
}
...
'</div>'+
  ...
  $("cf").addEventListener("submit",function(e){
    ...
    var data={
      ...
      convDate:fd.get("convDate")||"",
      incorpTime:fd.get("incorpTime")||"",
...
function bindAiParser(){
  ...
  btn.addEventListener("click",function(){
    ...
    $("aip-ok").addEventListener("click",function(){
      ...
      var prompt="Eres un experto en convocatorias del Real Madrid Cantera. Analiza el texto y devuelve SOLO JSON válido, sin markdown.\nHoy es "+today+". Fechas en formato YYYY-MM-DD. Si un dato no aparece usa null o [].\nJSON schema:\n{\"tipoDocumento\":\"prelista|lista_definitiva|otro\",\"selectionType\":\"madrilena|espanola|internacional\",\"convType\":\"definitiva|provisional\",\"selectionCategory\":\"sub12|sub14|sub15|sub16|sub17|sub18|sub19|sub20|sub21|sub23|abs\",\"pais\":\"pais si internacional sino null\",\"title\":\"titulo descriptivo\",\"startDate\":\"YYYY-MM-DD\",\"endDate\":\"YYYY-MM-DD\",\"rival\":\"rivales\",\"location\":\"lugar\",\"conc\":{\"date\":\"YYYY-MM-DD\",\"time\":\"HH:MM\",\"lugar\":\"lugar citacion\",\"hotel\":\"alojamiento\"},\"traslado\":{\"date\":\"YYYY-MM-DD\",\"time\":\"HH:MM\",\"lugar\":\"punto salida\",\"transporte\":\"vuelo/tren ida\"},\"vuelta\":{\"date\":\"YYYY-MM-DD\",\"time\":\"HH:MM\",\"lugar\":\"punto salida vuelta\",\"transporte\":\"vuelo/tren vuelta\"},\"matches\":[{\"date\":\"YYYY-MM-DD\",\"time\":\"HH:MM\",\"rival\":\"rival\",\"matchType\":\"amistoso|oficial|entrenamiento\"}],\"notes\":\"obs\",\"players\":[\"NOMBRE APELLIDO\"]}\ntipoDocumento: pon \"prelista\" si el texto/imagen menciona prelista, preconvocatoria, preseleccionados o similar. Pon \"lista_definitiva\" si menciona lista definitiva, lista final, convocatoria definitiva o similar. Si no está claro, pon \"otro\".\nTEXTO:\n"+text;
      ...
      fetchPromise.then(function(data){
        ...
        setTimeout(function(){
          ...
          if(parsed.tipoDocumento==="prelista"){var pcEl=form.querySelector('[name="preconvDate"]');if(pcEl&&!pcEl.value)pcEl.value=new Date().toISOString().slice(0,10);}
          if(parsed.tipoDocumento==="lista_definitiva"){var cdEl=form.querySelector('[name="convDate"]');if(cdEl&&!cdEl.value)cdEl.value=new Date().toISOString().slice(0,10);}
...
function renderJugadores(){
  ...
  var teamPills='<div class="view-toggle" style="flex-wrap:wrap;height:auto">'+
    '<button class="vtbtn'+(S.jugTeam===""?" on":"")+'" data-jt="">Todos</button>'+
    allTeams.map(function(t){return'<button class="vtbtn'+(S.jugTeam===t.name?" on":"")+'" data-jt="'+esc(t.name)+'">'+esc(t.name)+'</button>';}).join("")+
    '</div>';
  var h='<div class="vh"><h1 class="vt">Plantillas</h1><span class="vs">'+filtered.filter(function(p){return p.active;}).length+" activos</span></div>"+
    ...
    teamPills+
    '<button class="btn btn-gold" id="btn-add-player" style="width:100%;margin-bottom:12px;margin-top:10px">+ Añadir jugador</button>';
  ...
  var addBtn=$("btn-add-player");if(addBtn)addBtn.addEventListener("click",openPlayerAddChoice);
}
...
function openPlayerAddChoice(){
  var mo=document.createElement("div");mo.className="mo";
  mo.innerHTML='<div class="modal"><button class="mcl" id="pac-close">×</button>'+
    '<div class="mtitle">Añadir jugador</div>'+
    '<div style="display:flex;gap:8px;margin-top:12px">'+
    '<button class="btn btn-primary" id="pac-single" style="flex:1">Uno solo</button>'+
    '<button class="btn btn-ghost" id="pac-list" style="flex:1">En lista</button>'+
    "</div></div>";
  document.body.appendChild(mo);
  function closeMo(){if(mo.parentNode)mo.parentNode.removeChild(mo);}
  $("pac-close").addEventListener("click",closeMo);
  mo.addEventListener("click",function(e){if(e.target===mo)closeMo();});
  $("pac-single").addEventListener("click",function(){closeMo();openPlayerAdd();});
  $("pac-list").addEventListener("click",function(){closeMo();openPlayerAddBulk();});
}
